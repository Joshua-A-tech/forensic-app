...
function Check-Command($name) {
    if (-not (Get-Command $name -ErrorAction SilentlyContinue)) {
        Write-Error ('Required command ''{0}'' not found. Please install and authenticate before running this script.' -f $name)
        exit 1
    }
}
...
Write-Host ('Creating GitHub repository ''{0}''...' -f $repoName)
...
if ($LASTEXITCODE -ne 0) {
    Write-Error 'gh repo create failed. Ensure you are authenticated with gh (run ''gh auth login'') and try again.'
    exit 1
}
